    ...
    public OrderModel createOrder(OrderModel order) {
        try{
            ...
            if(order.getCost() <= whoOrdered.getMoney()){
                ...
                for(int i = 0; i < order.getItems().getOrderedProducts().length; i++){
                    orderInString += "<img width=200 height=100 src='" + productsRepository.findById(order.getItems().getOrderedProducts()[i].getProductId()).get().getImage() + "'>" + "<br>" + productsRepository.findById(order.getItems().getOrderedProducts()[i].getProductId()).get().getProductName() + "<br>" + productsRepository.findById(order.getItems().getOrderedProducts()[i].getProductId()).get().getPrice() + "<br>" + productsRepository.findById(order.getItems().getOrderedProducts()[i].getProductId()).get().getQuantity() + "<br>" + productsRepository.findById(order.getItems().getOrderedProducts()[i].getProductId()).get().getDetails();;
                }
                try {
                    SendEmail.sendmail(
